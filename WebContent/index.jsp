...
<body>
	<%
			...
			passwordString = request.getParameter("passwd");
			
			MyDB myDB = new MyDB();
			//myDB.myDBInit(8, "user", "000000", "aiplan");
			myDB.myDBInit(8, "x202lxy1mo", "30j5j1z25iiyyi2zxz3511xk215k1m4z3z13mlh0", "app_dxxtwxweb");
			ResultSet resultSet = myDB.myDBSql("select * from shop where shop='"+userString+"';");
			
			while(resultSet.next()){
				passwdString = resultSet.getString("password");
			}
			myDB.myDBClose();
			
			if(passwordString != null){
			if(!passwordString.equals(passwdString)){
				String hint = "用户名不存在或密码错误";
				request.getRequestDispatcher("login.jsp?hint="+URLEncoder.encode(hint,"UTF-8")).forward(request, response);
			}}else{
				String hint = "未登录状态";
				request.getRequestDispatcher("login.jsp?hint="+URLEncoder.encode(hint,"UTF-8")).forward(request, response);
			}
	%>
		...
		<nav id="sidebar" class="sidebar">
          ...
          <span class="align-middle">WX Server</span>
        </a>
